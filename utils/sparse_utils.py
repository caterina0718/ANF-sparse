...
def compute_sparsity_per_layer(global_sparsity, neuron_layers, keep_dense, closeness=0.5, method='ER', input_layer_sparsity=-1.):
    ...
    :param method: str, 'new' or 'ER' or 'uniform' or 'sparse_input' or 'ER_input' or 'sparse_output' or 'ER_output' or 'inverse_ER' or 'random' or 'dynamic'. 
                  ER is from Mocanu et al. 2018: https://www.nature.com/articles/s41467-018-04316-3                   uniform gives each layer same sparsity level (which needs to be higher than the desired global sparsity if you want some dense layers as well.) 
                  new is based on ER, but uses 'closeness' as a hyperparamter, such that when closeness=0.5, the probability of new is 0.3ER
                  sparse_input is computed as the index of the layer over the total number of layers
                  exp_input ensures that the closer a layer is to the input layer, the sparser it is; it uses an exponential distribution to calculate the probability
                  sparse_output is computed as 1 - (the index of the layer / the total number of layers)
                  exp_output ensures that the closer a layer is to the output layer, the sparser it is; it uses an exponential distribution to calculate the probability
                  inverse_ER layers with less neurons will be sparser (assuming that the input and output layers have less neurons, most of the sparsity will appear on these layers)
                  normal uses a normal distribution for creating sparse hidden layers and dense input and output layers
                  random sets sparsity values at random for each layer, while maintaining the given global sparsity (as an average)
                  dynamic 
    ...
    #create random probabilities vector with average at global density
    rand_sparsity = np.random.uniform(0.0, 1.0, len(neuron_layers)-1) 
    rand_sparsity = rand_sparsity / np.sum(rand_sparsity) * global_density * (len(neuron_layers)-1)
    
    #create normal distribution parameters
    mu = int(len(neuron_layers) / 2)
    sigma = 0.8 #1.3 #1.4 #1.8 
    extra = 0.16 #-0.12 #0.04 #0.09 #(97%) #0.49(80%) #0.65 #0.7 #0.77  
    ...
    for c_layer_idx, c_layer_dense in enumerate(keep_dense):
        if c_layer_dense:
            ...
            if c_layer_idx == 0 and input_layer_sparsity > 0:
                ...
                if method == 'new':
                    prob = 0.3 / ((neuron_layers[c_layer_idx] * neuron_layers[c_layer_idx + 1]) ** closeness)
                elif method == 'ER':
                    prob = (neuron_layers[c_layer_idx] + neuron_layers[c_layer_idx + 1]) \
                           / (neuron_layers[c_layer_idx] * neuron_layers[c_layer_idx + 1])
                elif method == 'uniform':
                    prob = global_density
                elif method == 'sparse_input':
                    prob = (c_layer_idx+1)/len(neuron_layers)  #+1 to ensure that the last layer is dense
                elif method == 'exp_input':
                    prob = 1 - 0.2 / ((c_layer_idx+1) ** 1.5)
                elif method == 'sparse_output':
                    prob = 1 - c_layer_idx/len(neuron_layers)  
                elif method == 'exp_output':
                    prob = 0.2 / ((c_layer_idx+1) ** 1.5)
                elif method == 'inverse_ER':
                    prob = 1 - (0.3 * (neuron_layers[c_layer_idx] + neuron_layers[c_layer_idx + 1])/ (neuron_layers[c_layer_idx] * neuron_layers[c_layer_idx + 1]))
                elif method == 'normal':
                    prob = 1- (extra + (1 / (sigma * (2 * np.pi) ** 0.5)) * np.e ** (-0.5 * ((c_layer_idx - mu) / sigma) ** 2)) 
                elif method == 'random':
                    prob = rand_sparsity[c_layer_idx]
                #elif method == 'dynamic':
                    #prob = 
                else:
    ...
    for c_layer_idx, keep_prob in enumerate(keep_probs):
        ...
        if keep_prob > 1:
            ...
            print("Probab:", keep_prob, ", threshold:", prob_for_one_connection)
